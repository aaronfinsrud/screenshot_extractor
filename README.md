# screenshot_extractor
A script created to extract full resolution images from an .xlsx file

## Instructions:
Read the functionality specs below and prepare your .xlsx file accordingly.
Double click on the screenshot extractor Python script. A file selector window will appear.
Select the .xlsx file you want to extract screenshots from.
Press enter.
A .zip file will be created within the same directory as the .xlsx file.
Unzip the file to view/edit the images.

## Functionality Specs:
All sheets are exported into one .zip file.
To match one screenshot to multiple keys, you can either use ⌘+c/ctrl+c to copy the screenshot to another row/sheet individually OR you can adjust the height of the image (dragging the bottom or upper edge) so that the screenshot spans the keys you want it to match with.
If key column is left empty, the exported screenshots will be named no_key1, no_key2, etc.
Column headers should be located in row 1
Key column should be labelled "Key", "key", "Keys", "keys" without any leading or trailing spaces
Only use one column to record the keys.
If the script detects that two images are going to share the same key (i.e. if two images are overlapping or horizontally aligned with the same cell), it will throw an error that reads
Exception: Make sure each key is unique and try again!
Press <return> to exit...
